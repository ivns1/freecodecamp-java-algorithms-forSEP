...
public class TernarySearchDiscrete {
  ...
  // search. Remember that your function must be a discrete and an unimodal
  ...
  static double discreteTernarySearch(int lo, int hi) {
    while (lo != hi) {
      TernarySearchCoverageTool.trackBranch104Hit(); // Track branch entering while loop

      if (hi - lo == 1) {
        TernarySearchCoverageTool.trackBranch105Hit(); // Track branch hi - lo == 1
        return Math.min(f(lo), f(hi));
      }
      if (hi - lo == 2) {
        TernarySearchCoverageTool.trackBranch106Hit(); // Track branch hi - lo == 2
      ...
      if (Math.abs(res1 - res2) < EPS) {
        TernarySearchCoverageTool.trackBranch107Hit(); // Track branch Math.abs(res1 - res2) < EPS
        lo = mid1;
        hi = mid2;
      } else if (res1 > res2) {
        TernarySearchCoverageTool.trackBranch108Hit(); // Track branch res1 > res2
        lo = mid1;
      } else {
        TernarySearchCoverageTool.trackBranch109Hit(); // Track branch res1 <= res2
        hi = mid2;
      }
    }
  ...
  public static void main(String[] args) {
    ...
    // whole interval of our function.
